...
- **Отмены стартов** (`kind=cancellations`,
  `services/cancellation_notification_service.py`): изменение отмены ближайшего
  старта уходит тем, для кого площадка своя — домашняя локация либо визит
  (пробежка или волонтёрство) за последний год; площадка считается одной точкой
  сквозь платформы по каноническому ключу каталога. Фильтр систем —
  `user_notification_prefs.cancellation_platforms` (пустой список = все,
  миграция 101). Зовётся из общего входа
  ...
  Ключ повтора — площадка + состояние + номер недели.
