 ...
 * дважды, второй палец не отпустил и повёл вниз — карта плавно приближается,
 * повёл вверх — отдаляется. Одной рукой, без щипка (просьба Дмитрия
 * 22.08.2026); направление — как у Яндекса (правка 23.08.2026).
 *
...
export function addDoubleTapDragZoom(map: L.Map): () => void {
  ...
  const onTouchMove = (event: TouchEvent) => {
    ...
    lastMoveAt = now;
    // Ведём вверх — ОТДАЛЯЕМ. Так в Яндекс.Картах и в приложении 5 вёрст:
    // палец тянет карту «от себя», она уходит вдаль и становится мельче.
    // Обратное направление ощущается как ошибка (репорт Дмитрия 23.08.2026).
