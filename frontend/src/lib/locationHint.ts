 ...
 *
 * Переход «Локация → Журнал протоколов» идёт без перезагрузки (pushState), но
 * страница собирается заново (hooks/useEntryKey), а после F5 или при заходе
 * по ссылке — с нуля. Пока не приехал ответ API, названия локации ещё нет —
 * подпункт сайдбара на долю секунды исчезал и появлялся снова (репорт
 * Дмитрия 11.08.2026). Имя последней открытой локации держим в
 * sessionStorage и читаем синхронно на первом рендере — сайдбар рисуется
 * сразу правильным.
 */
