...
/** Суббота недели (пн–вс) с этой датой — та же логика, что в календаре-хитмэпе
 * «Пробежек» личного кабинета: воскресный старт (RunPark или перенос с рабочей
 * субботы) попадает в клетку субботы, которая только что прошла. */
function weekSaturdayIso(isoDate: string): string {
  ...
  value.setDate(value.getDate() + (5 - ((value.getDay() + 6) % 7)));
  // Края года: активность буднего хвоста (пн 30.12 и позже) закрывалась бы
  // субботой уже СЛЕДУЮЩЕГО года, а воскресенье 01.01 — субботой предыдущего;
  // ни той, ни другой колонки среди суббот выбранного года нет, и отметка молча
  // выпадала из матрицы, хотя «За год» её считает (бэкенд режет по календарному
  // году). Прижимаем такую неделю к ближайшей субботе своего года.
  if (value.getFullYear() > sourceYear) {
    value.setDate(value.getDate() - 7);
  } else if (value.getFullYear() < sourceYear) {
    value.setDate(value.getDate() + 7);
  }
