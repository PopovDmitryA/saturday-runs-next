...
function RotationCell({ role }: { role: OrganizerTeamRole }) {
  ...
  return (
    <span>
      ...
      <span className="muted">({role.network_rotation_pct}% в системе)</span>
    </span>
  );
}
              ...
              <table className="data-table org-svod-table">
                <thead>
                  <tr>
                    ...
                    <ColumnHeader
                      label="Bus-фактор"
                      hint="Считаем так: людей роли выстраиваем по числу выходов за год и смотрим, сколько самых активных ВМЕСТЕ закрывают 80% всех выходов. Пример: секундомер выходил 50 раз, из них 42 раза — один и тот же человек. 42 из 50 — это больше 80%, значит bus-фактор 1: не придёт он — роль закрывать некому"
                      {...sortProps("bus")}
                    />
                    <ColumnHeader
                      label="Людей"
                    ...
                    <ColumnHeader
                      label="Ротация"
                      hint="Разных людей ÷ волонтёрств в роли. 100% — каждый раз новый человек, низкий процент — одни и те же. Рядом — средняя ротация этой же роли по системе; цвет — как наша смотрится на её фоне"
                      {...sortProps("rotation")}
                    />
                  </tr>
                </thead>
