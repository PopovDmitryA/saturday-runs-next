...
import { getCurrentUser, getDashboard, listRuns, type DashboardResponse, type RunItem, type User } from "../../lib/api";
import {
  getShareBackground,
...
function ShareContent() {
  ...
  useEffect(() => {
    if (step !== "period" || !customSourceImage || !isCustomBackground || customBackgroundImage) {
      return;
    }
    let cancelled = false;
  ...
  }, [customBackgroundImage, customSourceImage, format.height, format.width, isCustomBackground, step]);
  ...
  const cardInput = useMemo(() => {
    ...
    if (periodId === "last_run" && !lastRun) {
      return null;
    }
    const metricRows = buildShareMetrics(dashboard.stats, effectiveFields, metricsContext, lastRun);
    const rowById = new Map(metricRows.map((row) => [row.id, row]));
    const layoutFields =
      periodId === "last_run" ? LAST_RUN_METRIC_FIELDS : effectiveFields;
    const metrics = layoutMetricZones(layoutFields, effectivePrimaryFieldId, (fieldId) =>
    ...
    return {
      ...
      backgroundImage,
      textColor,
      summaryLine: shareSummaryLine(metricsContext, lastRun),
    };
  }, [
    backgroundImage,
    dashboard,
    ...
    <AppShell title="Поделиться" activePath="/share">
      ...
      {!loading && !error && mode === "wizard" && !hasLinks && (
        <div className="card">
          ...
          <div className="actions-row">
            <button type="button" className="btn secondary" onClick={() => setMode("gallery")}>
              К примерам
            </button>
            <a className="btn primary" href="/dashboard#profiles">
              Перейти к привязке
      ...
      {!loading && !error && mode === "wizard" && hasLinks && (
        <>
          ...
          {step === "period" && (
            <section className="card share-section">
              ...
              <div className="actions-row">
                ...
                >
                  {periodId === "last_run" ? "Превью" : "Далее: пресет"}
                </button>
              </div>
            </section>
          )}
          ...
          {step === "fields" && presetId === "custom" && periodId !== "last_run" && (
            <section className="card share-section">
          ...
          {step === "preview" && cardInput && (
            <section className="card share-section">
              <h2 className="section-title">Превью</h2>
              <p className="muted share-section-lead">
                {format.title} · фон «{background.title}» · {periodTitle.toLowerCase()}
                {periodId !== "last_run" && presetId && presetId !== "custom"
              ...
              <div className="actions-row">
                <button
                  ...
                  onClick={() =>
                    setStep(
                      periodId === "last_run" ? "period" : presetId === "custom" ? "fields" : "preset",
                    )
                  }
                >
                  Назад
