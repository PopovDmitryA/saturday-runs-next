...
function OnboardingContent({ user }: { user: User }) {
  ...
  return (
    <AppShell title={firstName ? `Добро пожаловать, ${firstName}!` : "Добро пожаловать!"}>
      <section className="card onboarding-hero">
        <p className="onboarding-hero-lead">
          Соберём всю вашу беговую историю в одном кабинете. Найдите себя по имени — мы поищем сразу во
          всех системах: 5 вёрст, С95, parkrun и RunPark.
        </p>
        ...
        {linksLoaded && linkedCount > 0 && (
          <p className="onboarding-progress-note">
            Привязано систем: {linkedCount} из {ONBOARDING_PLATFORMS.length}. Можно продолжить поиск или
            перейти в кабинет.
          </p>
        )}
      </section>
...
export function OnboardingPage() {
  return <RequireAuth>{(user) => <OnboardingContent user={user} />}</RequireAuth>;
}
