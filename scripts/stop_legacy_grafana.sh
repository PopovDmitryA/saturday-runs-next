...
echo "=== Grafana ==="
if systemctl list-unit-files 2>/dev/null | grep -q '^grafana-server.service'; then
...
if systemctl list-unit-files 2>/dev/null | grep -q '^5verst_bot.service'; then
...
echo "=== проверка ==="
echo -n "  порт 9000: "; (ss -ltn | grep -q ":9000" && echo "ещё слушается (!)") || echo "закрыт"
