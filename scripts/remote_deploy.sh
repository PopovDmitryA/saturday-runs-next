...
# Маркер выкаченного коммита. Нужен домашнему серверу: он тоже исполняет код
# сайта (разбор очереди профилей) против ЭТОЙ базы, и обязан совпадать по схеме.
# Домашний сервер сам читает этот файл и подтягивается — деплою до него не
# дотянуться, он в домашней сети без входящего доступа.
#
# Почему маркер, а не `git rev-parse HEAD` на проде: HEAD на диске и код внутри
# контейнеров расходятся, если кто-то сделал `git pull` без деплоя. Так и было
# 02.09.2026: на диске лежал коммит на четыре миграции новее собранных образов.
# Маркер пишется ЗДЕСЬ, после успешной пересборки и health-check, поэтому
# отражает то, что реально работает.
date -u +%Y-%m-%dT%H:%M:%SZ > .deployed_at
git rev-parse HEAD > .deployed_sha
echo "маркер для домашнего сервера: $(cat .deployed_sha)"
