...
LAST_FETCH_KEY = "five_verst:fetch:last_completed_at"


def wait_for_turn(*, reason: str) -> None:
    """Enforce minimum spacing between any 5verst HTTP fetches (all processes)."""
    del reason
    settings = get_settings()
    redis = get_redis_client()
    min_interval = settings.five_verst_fetch_min_interval_seconds
    max_interval = settings.five_verst_fetch_max_interval_seconds
