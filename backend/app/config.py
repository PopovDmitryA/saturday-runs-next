...
class Settings(BaseSettings):
    ...
    # Одноразовые токены магической ссылки живут 5 минут, но строка остаётся в
    # базе навсегда: consume_magic_link только проставляет used_at. Держим их
    # месяц — на разбор жалоб «ссылка из бота не сработала» — и сносим. Копить
    # незачем, а копившиеся мешали удалять профиль при объединении аккаунтов.
    auth_one_time_token_retention_days: int = 30
    ...
    # Сколько строк таблицы /results/all/ разбирает ротация. None = всю
    # таблицу, и это правильное значение по умолчанию: страница УЖЕ скачана,
    # и резать её разбор незачем — сравнение хэшей идёт в памяти и в БД, сети
    # не касается. Пока стояло 20, правка старта тридцатинедельной давности
    # не находилась вовсе: он в окно не попадал. Число оставлено ради ручных
    # частичных прогонов (`/sync location <slug>`), где нужен быстрый заход.
    five_verst_location_batch_summaries_limit: int | None = None
    # Потолок ПЕРЕКАЧЕК протоколов за один заход по площадке. Нужен только
    # против первого прохода: если у площадки разом разошлись десятки сводок,
    # без потолка заход занял бы часы и перекрыл единственный воркер
    # five_verst (там же ходит часовой latest). Остаток не теряется — сводка
    # уже записана, а протокол под ней остался от прошлой версии, то есть
    # висит долг (`summary_hash_at_fetch ≠ summary_hash`), и это приоритетная
    # причина сверки: разберётся часами, а не следующим кругом.
    five_verst_location_protocol_fetch_limit: int = 10
    # Сколько площадок обходит одна ротация (расписание — раз в 4 часа, 6 раз
    # в сутки). 5 слагов × 6 прогонов ≈ 30 площадок в день, полный круг по
    # ~220 официальным локациям — около недели. По одной площадке за прогон
    # круг занимал больше месяца: поздняя правка прошедшего старта на
    # /results/all/ (доливка волонтёров, исправленное время) до месяца
    # оставалась незамеченной — сверка протоколов сравнивает протокол с нашей
    # же сводкой и такое расхождение не видит. Потолок нагрузки: страница
    # локации кэшируется на five_verst_location_refresh_interval_days, так что
    # прогон — это в основном 5 страниц /results/all/ плюс протоколы только
    # изменившихся стартов; общий интервал между любыми фетчами (20-30 с)
    # держит суммарный темп.
    five_verst_location_rotation_slugs_per_run: int = 5
    # Обход протоколов недели (app/sync/five_verst_week_sweep.py) — третья
    # страховка легаси-схемы: перекачать протоколы целой субботы и сверить с
    # базой, не глядя на сводку. Пачка × звеньев = 240 протоколов за прогон,
