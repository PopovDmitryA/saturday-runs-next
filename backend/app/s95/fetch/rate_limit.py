...
def wait_for_turn(*, reason: str) -> None:
    """Enforce minimum spacing between any S95 HTTP fetches (all processes)."""
    del reason
    settings = get_settings()
    redis = get_redis_client()
    min_interval = settings.s95_fetch_min_interval_seconds
    max_interval = settings.s95_fetch_max_interval_seconds
