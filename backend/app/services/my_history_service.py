...
def _collect_run_milestones(
    ...
    *,
    ...
    for run_number, (run, event, location, platform_code, participant) in enumerate(rows, start=1):
        ...
        # глобальный рекорд. Самая первая пробежка вообще — базовая точка, не
        # рекорд (веха «Первая пробежка»); ровно та же семантика, что у флага
        # is_global_pr в recalculate_cross_platform_personal_records.
        #
