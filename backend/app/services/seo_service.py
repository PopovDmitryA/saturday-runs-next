...
STATIC_PAGE_META: dict[str, PageMeta] = {
    ...
    "/login": _meta(
        ...
        "5 вёрст, С95, parkrun и RunPark, достижения. Вход через VK, Яндекс, Telegram, почту.",
        indexable=True,
    ),
    ...
    "/ratings/home-distance": _meta(
        "Дальность от дома — рейтинги run5k.run",
    ...
    "/ratings/regions": _meta(
        "Локации по регионам — рейтинги run5k.run",
        "Сколько локаций субботних пятёрок в каждом регионе России: 5 вёрст, S95 и "
        "RunPark в одной таблице, зарубежье — по странам.",
        indexable=True,
    ),
    # Трассы по трекам участников: пока фича закрыта (видит только админ),
    # поэтому в поиск не отдаём.
    "/ratings/courses": _meta(
        "Трассы локаций — рейтинги run5k.run",
...
def _login_body(meta: PageMeta) -> str:
    ...
    rows = [
        ...
        "    <p>Личный кабинет участника субботних пробежек. Вход через VK, Яндекс, "
        "Telegram или по коду на почту, "
