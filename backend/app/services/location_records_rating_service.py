...
def invalidate_location_records_rating_cache() -> None:
    """Ручной сброс снапшота — для разовых прогревов из админки и с сервера.

    Штатно снимок не удаляют, а подрезают ему TTL до двух минут после записи
    результатов — это делает mark_location_results_changed
    (app/services/location_freshness.py, с 21.09.2026). До того рейтинг жил до
    истечения TTL, и субботний рекорд доезжал до него только к протуханию;
    delete на каждый протокол синка был бы дороже — сотни холодных пересчётов
    подряд.
    """
    try:
