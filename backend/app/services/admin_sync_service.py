...
PIPELINES: dict[str, tuple[str, object, str]] = {
    ...
    "community": ("5v старты сообществ", sync_community_events_task, "five_verst"),
...
def enqueue_pipeline(name: str, *, location_slug: str | None = None) -> str:
    ...
    if key == "location":
        ...
        # Точечная перечитка локации руками — это всегда «покажите свежее прямо
        # сейчас», поэтому в приоритетную очередь. Фан-ауты по всем локациям
        # (enqueue_recent_protocols и компания) остаются в фоне.
        sync_location_task.apply_async(
            kwargs={"location_slug": location_slug}, queue=FIVE_VERST_FRESH_QUEUE
        )
