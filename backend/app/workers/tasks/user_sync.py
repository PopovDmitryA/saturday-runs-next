...
def user_sync_task(
    ...
    *,
    ...
    try:
        ...
        # Пока идёт этот синк, батчи 5 вёрст стоят на паузе между фетчами —
        # пользователь, нажавший «обновить», не ждёт окончания сверки истории.
        with five_verst_user_sync_context(
            ttl_seconds=get_settings().five_verst_user_sync_active_ttl_seconds
        ):
            job = run_user_sync(
                db,
                UUID(user_id),
