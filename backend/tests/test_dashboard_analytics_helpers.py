...
from app.saturday_week import max_saturday_streak, week_saturday
...
def test_week_saturday_maps_any_weekday_to_its_week_saturday() -> None:
    ...
    assert week_saturday(saturday) == saturday
    assert week_saturday(date(2026, 5, 20)) == saturday  # среда той же недели
    assert week_saturday(date(2026, 5, 18)) == saturday  # понедельник той же недели
    assert week_saturday(date(2026, 5, 24)) == saturday  # воскресенье, закрывает ту же неделю
    assert week_saturday(date(2026, 5, 17)) == date(2026, 5, 16)  # воскресенье прошлой недели
...
def test_week_saturday_keeps_sunday_transfer_in_its_own_saturday() -> None:
    ...
    assert week_saturday(date(2025, 11, 2)) == date(2025, 11, 1)
    assert max_saturday_streak(
...
def test_max_saturday_streak_merges_same_week_dates() -> None:
    ...
    assert max_saturday_streak(activity_dates) == 2
