...
def test_window_counted_from_snapshot_now_not_local_clock() -> None:
    """Окно отсчитываем от now_hour снимка, а не от часов веб-сервера.

    Метки — настоящий UTC, но точка отсчёта берётся из самого снимка: так окно
    не зависит от того, насколько разъехались часы прода и базы, и остаётся
    воспроизводимым для одного и того же снимка.
    """
