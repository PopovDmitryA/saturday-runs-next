...
> **С сентября 2026 очередь разбирается сама на домашнем сервере.** Тот же код
> запускает `scripts/home_queue_run.sh` по таймеру `pm-site-queue.timer` (раз в
> 20 минут от окончания прошлого прогона): httpx без браузера, капча решается
> CLIP, наружу ходим через пул VPN-выходов. Человек и `make parkrun` на Mac
> нужны только когда хочется разгрести очередь быстрее или посмотреть глазами.
> Важное отличие от Mac: домашний сервер исполняет код из `~/saturday-runs-next`,
> который догоняется до **выкаченного на прод** коммита, — правки доезжают туда
> через деплой, а не со следующего запуска.
