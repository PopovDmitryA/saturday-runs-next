...
| `QUIET=1` | прячет INFO-шум библиотек (DB flush, httpx, page-load), оставляет строку прогресса `N/total`, `→ ссылка + имя`, предупреждения и итог. |
...
| `FAST_DELAY=N` | только с `NO_BROWSER`: пауза между запросами, N секунд с джиттером ±30%. Действует и человек↔человек, и человек↔локация. По умолчанию 3. |
