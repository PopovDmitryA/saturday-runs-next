...
### Отмена ближайшего старта у S95 (с 27.08.2026)

До Иванова считалось, что недельных отмен S95 не публикует вовсе, и любая
неработающая карточка означала закрытие площадки. 27.08.2026 Иваново отменило
старт 29 августа — и оказалось, что система показывает это тремя разными
способами:

| Где | Как выглядит | Что даёт |
|-----|--------------|----------|
| `/events/{slug}` (HTML) | `div.alert.alert-danger`: «Внимание! Ближайший старт отменён.» + `<strong>Причина:</strong> …` | Единственное место с текстом причины |
| `/events/` (HTML) | карточка `div.card-with-border.border-danger` + красная накладка `bg-danger` | Видно списком, без текста |
| `/events.json` | `"active": false` | Дёшево, но неотличимо от закрытия |
| `/pages.json` | карточки **нет вовсе** | Ловушка: обход только по `pages.json` пропускал площадку целиком |

Отсюда устройство кода:

- `app/s95/api_client.fetch_all_locations` собирает список из `pages.json` **и**
  `events.json`: первый знает про разъездные серии без координат, второй — про
  неработающие площадки. Активность — строгое «и» двух источников.
- `app/sync/s95_location_status.resolve_s95_location_status` для каждой
  неработающей карточки грузит её страницу и по плашке решает: отмена
  (`is_cancelled` + `cancel_reason`) или закрытие (`is_paused`). Страница
  грузится только для таких карточек — обычно их ноль.
- `app/sync/s95_cancellations` — отдельный наблюдатель в beat 4 раза в сутки
  (`s95_sync.watch_cancellations`, 00:40/06:40/12:40/18:40 MSK). Реестр ходит
  раз в трое суток, а отмена живёт одну субботу. Наблюдатель трогает только
  `is_cancelled`/`cancel_reason`; «не действует» остаётся за синком реестра и
  правилом молчания, чтобы статусы не спорили.
- Изменение статуса «отмена» (в любой системе) уходит админу в Telegram —
  `app/services/location_cancellation_notify`. Сообщение только по факту
  изменения, одно на прогон.
- Причина видна на странице локации отдельной плашкой
  (`LocationCancellationNotice`); в БД это `locations.cancel_reason`
  (миграция 074), живёт ровно столько же, сколько сам флаг.
